...
from checkout.models import Checkout, CheckoutItem, Coupon, Shipping
...
# ==========================
@method_decorator(never_cache, name='dispatch')
class ShippingEditView(LoginRequiredMixin, generic.View):
    login_url = 'login'
    ...
    def post(self, request, id, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Shipping address edit successfully.')
            return redirect('checkout-view')
...
class CheckoutView(LoginRequiredMixin, generic.View):
    ...
    def post(self, request):
        ...
        # Safely check if coupon exists without raising a 404 exception
...
class CheckoutListsView(LoginRequiredMixin, generic.View):
    ...
    def get(self, request):
        checkouts = Checkout.objects.filter(user=request.user)\
            .prefetch_related('items__product__galleries', 'items__variant')\
            .order_by('-created_at')
